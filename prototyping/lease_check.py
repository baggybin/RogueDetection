...
class lease_check:
        ...
        def parse_timestamp(self, raw_str):
        ...
        def timestamp_is_ge(self, t1, t2):
                if t1 == 'never':
                        return True
                elif t2 == 'never':
                        return False
        ...
        def timestamp_is_lt(self, t1, t2):
                if t1 == 'never':
                        return False
                elif t2 == 'never':
                        return t1 != 'never'
        ...
        def timestamp_is_between(self, t, tstart, tend):
                return self.timestamp_is_ge(t, tstart) and self.timestamp_is_lt(t, tend)
        
        
        def parse_hardware(self, raw_str):
                tokens = raw_str.split()
                if len(tokens) == 2:
                        return tokens[1]
                else:
        ...
        def strip_endquotes(self, raw_str):
                return raw_str.strip('"')
        
        
        def identity(self, raw_str):
                return raw_str
        
        
        def parse_binding_state(self, raw_str):
                tokens = raw_str.split()
                if len(tokens) == 2:
                        return tokens[1]
                else:
        ...
        def parse_next_binding_state(self, raw_str):
                tokens = raw_str.split()
                if len(tokens) == 3:
                        return tokens[2]
                else:
        ...
        def parse_rewind_binding_state(self,raw_str):
                tokens = raw_str.split()
        ...
        def parse_leases_file(self, leases_file):
                valid_keys = {
                        'starts':               self.parse_timestamp,
                        'ends':                 self.parse_timestamp,
                        'tstp':                 self.parse_timestamp,
                        'tsfp':                 self.parse_timestamp,
                        'atsfp':                self.parse_timestamp,
                        'cltt':                 self.parse_timestamp,
                        'hardware':             self.parse_hardware,
                        'binding':              self.parse_binding_state,
                        'next':                 self.parse_next_binding_state,
                        'rewind':               self.parse_rewind_binding_state,
                        'uid':                  self.strip_endquotes,
                        'client-hostname':      self.strip_endquotes,
                        'option':               self.identity,
                        'set':                  self.identity,
                        'on':                   self.identity,
                        'abandoned':            None,
                        'bootp':                None,
        ...
        def round_timedelta(self, tdelta):
        ...
        def lease_is_active(self, lease_rec, as_of_ts):
                return self.timestamp_is_between(as_of_ts, lease_rec['starts'],
                        lease_rec['ends'])
        
        
        def ipv4_to_int(self, ipv4_addr):
                parts = ipv4_addr.split('.')
                return (int(parts[0]) << 24) + (int(parts[1]) << 16) + \
                        (int(parts[2]) << 8) + int(parts[3])
        
        
        def select_active_leases(self, leases_db, as_of_ts):
                retarray = []
                sortedarray = []
                ...
                for ip_address in leases_db:
                        ...
                        if self.lease_is_active(lease_rec, as_of_ts):
                                ip_as_int = self.ipv4_to_int(ip_address)
